"""
Simple Flask web app to view Kitsu Output File metadata.
...
def view_output_file(output_file_id):
    ...
    try:
        ...
        # Get preview file - fetch from the task where we uploaded the proxy
        preview_file_id = None
        has_preview = False
        
        try:
            # Get the task associated with this output file
            task_type_id = output_file.get('task_type_id')
            ...
            if task_type_id and entity_id:
                ...
                for task in tasks:
                    if task.get('task_type_id') == task_type_id:
                        # Use the CORRECT Gazu function
                        previews = gazu.files.get_all_preview_files_for_task(task)
                        
                        if previews:
                            # Get the most recent preview (last one in the list)
                            latest_preview = previews[-1]
                            preview_file_id = latest_preview.get('id')
                            has_preview = True
                            print(f"✅ Found preview: {preview_file_id}")
                            break
                        else:
                            print(f"⚠️ Task found but no previews: {task.get('id')}")
        except Exception as e:
            print(f"❌ Could not fetch preview: {e}")
            import traceback
            traceback.print_exc()
...
def get_preview(preview_file_id):
    ...
    try:
        print(f"🎥 Fetching preview: {preview_file_id}")
        
        # Get the preview file metadata
        preview_file = gazu.files.get_preview_file(preview_file_id)
        print(f"📄 Preview info: {preview_file.get('original_name')} - Extension: {preview_file.get('extension')}")
        ...
        print(f"📥 Downloading preview to: {temp_path}")
        ...
        print(f"❌ Exception fetching preview: {e}")
        import traceback
        traceback.print_exc()
        return str(e), 500
...
if __name__ == '__main__':
    ...
    print("📄 View files at: http://localhost:5000/output-file/<id>\n")
